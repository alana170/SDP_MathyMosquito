...
/* updates high score 
   takes in the score for that round 
...
/* function for all motion for addition round
...
void flyswatter()
{
  ...
  // Draw a image in the bottom left side
  flyswatter.Draw(0, 140);
  // x and y position of the flyswatter
  int xf = 320;
  int yf = 140;
  // create another flyswatter upside down positioned in the top right
  ...
  mosquito.Draw(x1, y1);
  LCD.Update();
  int disappear = 0; // the token should not disappear if it = 0

  // main while loop
  while (true)
  {
    // makes sure flyswatters keep regenerating when going off screen
    if (xf < 0)
    {
      xf = 320;
    }
    ...
    // Change y coordinate of mosquito to go down
    y1 += 5;
    ...
    if (xToken <= 4)
    {
      ...
      disappear = 0; // the token should not disappear if it = 0
      // clear old tokens
      LCD.Clear(WHITE);
    ...
    while ((!LCD.Touch(&x, &y)))
    {
      ...
      xToken = xToken - 10;
      // if disappear is 0, redraw token
      if (disappear == 0)
      {
      ...
      // so xf and xudf (position of flyswatter and upside down flyswatter) don't go negative
      if (xf < 0)
      {
        xf = 320;
      }
      ...
      Sleep(0.1);
      // if mosquito collides with bottom flyswatter
      if (((110 <= xf) && (180 >= xf) && (y1 > 130)))
      {
        Sleep(0.1);
        // pass to collision function
        collision();
      }
      // if mosquito collides with the top flyswatter
      if ((100 <= xudf) && (180 >= xudf) && ((y1 < 50)))
      {
        Sleep(0.1);
        collision();
      }
      // for collision with token
      if ((yToken >= y1 - 20) && (yToken <= y1 + 20) && (xToken >= x1 - 20) && (xToken <= x1 + 20) && (disappear == 0))
      {
        ...
        disappear = 1;
        // go to coin collect function to update score
        coincollect();
      }
    }
    // Touch - use x and y
    ...
    udflyswatter.Draw(xudf, yudf);
    if (disappear == 0)
    { // draw token only if it has not been collided with yet
      token.Draw(xToken, yToken);
      LCD.Update();
      Sleep(0.1);
    }
  ...
  udflyswatter.Close();
  token.Close();
} // end of the flyswatter function
...
/* function that draws the title page and waits for user to touch screen to go to the menu */
void title()
{
  high_score = 0;
  Sleep(2);
...
/* function that randomly generates addition math problem */
void math()
{
  // math equation
  LCD.SetFontColor(RED);
  ...
  else
  {
    ...
    // update high score
    updateHighScore(score_now);

    // display scores
    LCD.WriteAt("Score:", 50, 100);
    LCD.WriteAt(score_now, 150, 100);

    LCD.WriteAt("High Score:", 50, 150);
    LCD.WriteAt(high_score, 200, 150);
...
// function that updates score when coin is collected
int coincollect()
{
...
void menu()
{
  ...
  float x_pos, y_pos, x_trash, y_trash;
  LCD.Clear(WHITE);
  // make the light blue box background
  ...
  while (true)
  {
    ...
    // check where on the screen the user selected
...
/* upper right corner menu button so that you 
can navigate back to menu from different pages */
void MenuButton()
{
...
void ViewStats()
{
  ...
  LCD.WriteRC(high_score, 6, 1);
  // option to return to menu
  MenuButton();
} // end of function

/* when quit is selected on the menu
   ask if they want to quit and leave program 
   if yes is selected */ 
void Quit()
{
  LCD.Clear(SKYBLUE);
  ...
  while (true)
  {
    ...
    while (!LCD.Touch(&x_pos, &y_pos)){};
    while (LCD.Touch(&x_trash, &y_trash)){};
    // closes the game if no is selected
...
/* function that chooses level option 
   after play game is selected */
void chooselevel()
{
  ...
  else if (y_pos > 160 && y_pos < 240 && x_pos > 0 && x_pos < 320)
  {
    // play hard level
    flyswattermult();
  }
  ...
  {
    // press anywhere else to go back to menu
    menu();
  }
} // end of function
...
// collision function, passes into multiplication math problems
void collisionmult()
{
  LCD.Clear();
  // make the background red
